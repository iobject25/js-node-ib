...
Incoming.prototype._OPEN_ORDER = function () {
  ...
  if (version < 29) {
    ...
    order.lmtPrice = this.dequeueFloat() || DEFAULT_NUM;
  }

  if (version < 30) {
    order.auxPrice = this.dequeueFloat();
  } else {
    order.auxPrice = this.dequeueFloat() || DEFAULT_NUM;
  }
  ...
  if (version >= 9) {
    ...
    order.percentOffset = this.dequeueFloat() || DEFAULT_NUM;
    ...
    order.startingPrice = this.dequeueFloat() || DEFAULT_NUM;
    order.stockRefPrice = this.dequeueFloat() || DEFAULT_NUM;
    order.delta = this.dequeueFloat() || DEFAULT_NUM;
    order.stockRangeLower = this.dequeueFloat() || DEFAULT_NUM;
    order.stockRangeUpper = this.dequeueFloat() || DEFAULT_NUM;
  ...
  if (version >= 11) {
    order.volatility = this.dequeueFloat() || DEFAULT_NUM;
    ...
    if (version === 11) {
      ...
      order.deltaNeutralAuxPrice = this.dequeueFloat() || DEFAULT_NUM;
  ...
  if (version >= 13) {
    order.trailStopPrice = this.dequeueFloat() || DEFAULT_NUM;
  }

  if (version >= 30) {
    order.trailingPercent = this.dequeueFloat() || DEFAULT_NUM;
  }

  if (version >= 14) {
    order.basisPoints = this.dequeueFloat() || DEFAULT_NUM;
    order.basisPointsType = this.dequeueInt() || DEFAULT_NUM;
  ...
  if (version >= 29) {
    ...
    if (orderComboLegsCount > 0) {
      ...
      for (i = 0; i < orderComboLegsCount; ++i) {
        orderComboLeg = {};
        order.price = this.dequeueFloat() || DEFAULT_NUM;
  ...
  if (version >= 15) {
    if (version >= 20) {
      order.scaleInitLevelSize = this.dequeueInt() || DEFAULT_NUM;
      order.scaleSubsLevelSize = this.dequeueInt() || DEFAULT_NUM;
    } else {
      var notSuppScaleNumComponents = this.dequeueInt() || DEFAULT_NUM;
      order.scaleInitLevelSize = this.dequeueInt() || DEFAULT_NUM;
    }
    order.scalePriceIncrement = this.dequeueFloat() || DEFAULT_NUM;
  }

  if (version >= 28 && order.scalePriceIncrement > 0.0 && order.scalePriceIncrement !== DEFAULT_NUM) {
    order.scalePriceAdjustValue = this.dequeueFloat() || DEFAULT_NUM;
    order.scalePriceAdjustInterval = this.dequeueInt() || DEFAULT_NUM;
    order.scaleProfitOffset = this.dequeueFloat() || DEFAULT_NUM;
    order.scaleAutoReset = this.dequeueBool();
    order.scaleInitPosition = this.dequeueInt() || DEFAULT_NUM;
    order.scaleInitFillQty = this.dequeueInt() || DEFAULT_NUM;
  ...
  if (version >= 16) {
    ...
    if (orderState.initMargin == Number.MAX_VALUE) {
      orderState.initMargin = DEFAULT_NUM;
    }
    if (orderState.maintMargin == Number.MAX_VALUE) {
      orderState.maintMargin = DEFAULT_NUM;
    }
    if (orderState.equityWithLoan == Number.MAX_VALUE) {
      orderState.equityWithLoan = DEFAULT_NUM;
    }
    orderState.commission = this.dequeueFloat() || DEFAULT_NUM;
    orderState.minCommission = this.dequeueFloat() || DEFAULT_NUM;
    orderState.maxCommission = this.dequeueFloat() || DEFAULT_NUM;
...
Incoming.prototype._TICK_OPTION_COMPUTATION = function () {
  ...
  var optPrice = DEFAULT_NUM;
  var pvDividend = DEFAULT_NUM;
  var gamma = DEFAULT_NUM;
  var vega = DEFAULT_NUM;
  var theta = DEFAULT_NUM;
  var undPrice = DEFAULT_NUM;
