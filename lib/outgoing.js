...
Outgoing.prototype.placeOrder = function (id, contract, order) {
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.SCALE_ORDERS) {
    if (order.scaleInitLevelSize !== DEFAULT_NUM ||
        order.scalePriceIncrement !== DEFAULT_NUM) {
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.SCALE_ORDERS2) {
    if (order.scaleSubsLevelSize !== DEFAULT_NUM) {
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.SCALE_ORDERS3) {
    if (order.scalePriceIncrement > 0 && order.scalePriceIncrement !== DEFAULT_NUM) {
      if (order.scalePriceAdjustValue !== DEFAULT_NUM ||
          order.scalePriceAdjustInterval !== DEFAULT_NUM ||
          order.scaleProfitOffset !== DEFAULT_NUM ||
          order.scaleAutoReset ||
          order.scaleInitPosition !== DEFAULT_NUM ||
          order.scaleInitFillQty !== DEFAULT_NUM ||
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.ORDER_COMBO_LEGS_PRICE &&
    ...
    if (_.isArray(order.orderComboLegs)) {
      order.orderComboLegs.forEach(function (orderComboLeg) {
        if (orderComboLeg.price !== DEFAULT_NUM) {
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.TRAILING_PERCENT) {
    if (order.trailingPercent !== DEFAULT_NUM) {
  ...
  if (this._controller._serverVersion < C.MIN_SERVER_VER.ORDER_COMBO_LEGS_PRICE) {
    args.push(order.lmtPrice === DEFAULT_NUM ? 0 : order.lmtPrice);
  } else {
    args.push(_nullifyMax(order.lmtPrice));
  }
  if (this._controller._serverVersion < C.MIN_SERVER_VER.TRAILING_PERCENT) {
    args.push(order.auxPrice === DEFAULT_NUM ? 0 : order.auxPrice);
  } else {
  ...
  if (this._controller._serverVersion >= 26) { // Volatility orders
    ...
    if (this._controller._serverVersion === 26) {
      // Volatility orders had specific watermark price attribs in server version 26
      lower = (order.orderType === 'VOL' ? order.stockRangeLower : DEFAULT_NUM);
      upper = (order.orderType === 'VOL' ? order.stockRangeUpper : DEFAULT_NUM);
      args.push(_nullifyMax(lower));
      args.push(_nullifyMax(upper));
    }
